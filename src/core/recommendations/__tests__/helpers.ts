import type { RoundHole, RoundWithHoleScores } from "../types";
...
export function makeHole(overrides: Partial<RoundHole> & { hole_number: number }): RoundHole {
  ...
  const stroke_index = overrides.stroke_index ?? DEFAULT_INDEXES[overrides.hole_number - 1] ?? overrides.hole_number;
  return {
...
export interface MakeRoundOptions {
  ...
  slope_rating?: number;
  /** Per-hole override factory; receives hole_number 1..18 and the default hole. */
...
export function makeRound(opts: MakeRoundOptions = {}): RoundWithHoleScores {
  ...
  return {
    ...
    course_id: 1,
    tee_id: 1,
    played_at: opts.played_at ?? `2026-${String(id).padStart(2, "0")}-01T12:00:00.000Z`,
...
export function makeRounds(count: number, factory: (i: number) => MakeRoundOptions): RoundWithHoleScores[] {
